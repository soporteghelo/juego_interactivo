...
/**
 * TABLERO DE GESTION AESA (SSOMA) — md: panel verde con borde rojo/blanco ~80×60cm, con
...
export const meta = {
  id: 'tablero_gestion',
  nombre: 'Tablero de gestion (SSOMA/AESA)',
...
function texturaPanel() {
  ...
  ctx.fillText('GESTION SSOMA — AESA', c.width / 2, 38);
...
export function crear() {
  ...
  const frame = new THREE.Mesh(new THREE.BoxGeometry(0.86, 0.66, 0.02), MineMaterials.acero());
  ...
  g.userData.interactable = {
    object: panel,
    descriptor: {
      label: 'Revisar tablero de gestion (SSOMA / AESA)',
      onInteract: () => window.__mina?.bus.emit('ui:read', {
        title: 'TABLERO DE GESTION — SSOMA / AESA',
        body:
          ...
          'Respuesta a Emergencias. Valores AESA: Integridad, Bienestar y Seguridad, ' +
