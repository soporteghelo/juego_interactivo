...
/**
 * @param {{ doble?:boolean, conTableros?:boolean, seed?:number, conLuz?:boolean, irregular?:boolean }} opts
 *   conTableros: true  → nicho eléctrico con tableros (default)
 *   conTableros: false → nicho vacío (refugio peatonal, sin equipos)
 *   conLuz:      true  → añade el foco cálido interior (default). El refugio peatonal lo omite.
...
export function crear({ doble = false, conTableros = true, seed = 1, conLuz = true, irregular = false } = {}) {
  ...
  const aM = irregular ? 1.6 : 1.0;
  const eM = irregular ? 1.7 : 1.0;
  ...
  const gFondo = planoRocaTerrain(W, H, 28, 24, 0.32 * aM, 3.8, ox,      oy,      irregular ? 0.10 : 0);
  const fondo  = new THREE.Mesh(gFondo, mRoca);
  fondo.position.set(0, H / 2, -D);
  S.add(fondo);

  // ── PAREDES LATERALES ────────────────────────────────────────────
  const gLatIzq = planoRocaTerrain(D, H, 24, 24, 0.40 * aM, 4.0, ox+5.1, oy+2.3, irregular ? 0.12 : 0);
  const latIzq  = new THREE.Mesh(gLatIzq, mRoca);
  latIzq.position.set(-W / 2, H / 2, -D / 2);
  latIzq.rotation.y = -Math.PI / 2;
  S.add(latIzq);

  const gLatDer = planoRocaTerrain(D, H, 24, 24, 0.40 * aM, 4.0, ox+9.7, oy+6.1, irregular ? 0.12 : 0);
  ...
  const gTecho = planoRocaTerrain(W, D, 26, 22, 0.50 * aM, 4.2, ox+13.3, oy+4.7);
  ...
  const mAmp    = 0.28 * aM;   // desplazamiento de superficie (profundidad)
  const mEdge   = 0.32 * eM;   // perturbación de silueta — CLAVE para romper el rectángulo
  const mScl    = 4.0;
  ...
  // ── LUZ INTERIOR ─────────────────────────────────────────────────
  // El refugio peatonal (conLuz=false) NO lleva foco: en la via es un simple hueco de
  // resguardo, sin punto de luz propio que "manche" la galeria.
  if (conLuz) {
